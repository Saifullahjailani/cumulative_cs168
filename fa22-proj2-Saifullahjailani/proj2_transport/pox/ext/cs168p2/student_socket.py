...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    ## Start of Stage 1 ##
    syn = self.new_packet(False, None, True);
    syn.tcp.seq = self.snd.iss 
    self.tx(syn)
    self.state = SYN_SENT
    ## End of Stage 1 ##
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      seq_no, packet = self.rx_queue.peek()
      if seq_no |GT| self.rcv.nxt:
        self.set_pending_ack()
        break
      else:
        self.rx_queue.pop()
  ...
  def update_rto(self, acked_pkt):
    ...
    # first rtt measurment is whe srtt = 0
    if not self.srtt:
      self.srtt = RTT
      self.rttvar = RTT/2
    else:
  ...
  def handle_accepted_ack(self, seg):
    ...
    """
    ## Start of Stage 4 ##
    #since the we accept the ack we update una with the segments ack
    self.snd.una = seg.ack
    ## End of Stage 4    ##
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4 ##
    packets_in_flight =  snd.nxt |MINUS| snd.una
    remaining = min(len(self.tx_data), snd.wnd |MINUS| packets_in_flight)
    while remaining > 0:
      
      payloadSz = min(self.mss, remaining)
      payload = self.tx_data[:payloadSz]
      self.tx_data = self.tx_data[payloadSz:]


      num_pkts += 1
      bytes_sent += payloadSz
      remaining -= payloadSz 
  ...
  def check_timer_retx(self):
    ...
    if not self.retx_queue.empty():
      ...
      self.log.debug(f"check_timer_rtx before setting time in queue = {time_in_queue}")
      time_in_queue = self.stack.now - p.tx_ts
      self.log.debug(f"check_timer_rtx after setting time in queue = {time_in_queue}")
    ## End of Stage 8 ##
